import json
import os
import re
from bs4 import BeautifulSoup
# Repo root paths
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..'))
ASSETS_DIR = os.path.join(BASE_DIR, 'assets')
FILINGS_DIR = os.path.join(BASE_DIR, 'filings')
SEC_PDFS_DIR = os.path.join(BASE_DIR, 'sec_pdfs')


def merge_filings():
    base_dir = BASE_DIR
    data_path = os.path.join(ASSETS_DIR, "data.json")
    filings_dir = os.path.join(base_dir, "filings")
    
    with open(data_path, 'r', encoding='utf-8') as f:
        full_data = json.load(f)
        
    forms = full_data['forms']
    
    # Define Merges
    merge_map = {
        "10-K": ["10-KT", "10-K/A"],
        "10-Q": ["10-QT", "10-QT/A"],
        "11-K": ["11-KT"],
        "8-K": ["8-K12B", "8-K12G3", "8-K15D5"],
        "DEF 14A": ["DEFA14A", "DEFC14A", "DEFM14A", "DEFR14A", "DEFS14A", "DEF 14C", "PRE 14A", "PREC14A", "PREM14A", "PREN14A", "PRE 14C"],
        "424B4": ["424B1", "424B2", "424B3", "424B5", "424B7", "FWP"],
        "S-3": ["S-3ASR", "S-3MEF", "S-3D"],
        "Form 4": ["Form 3", "Form 5"],
        "SC 13D": ["SC 13G"],
        "NT 10-K": ["NT 10-Q", "NT 20-F", "NT 11-K", "NT-NCSR", "NT 10-K/A", "NT 10-Q/A"],
        "Form 15": ["15-12B", "15-12G", "15-15D", "15F-12B", "15F-12G", "Form 25", "25-NSE", "34-12H"],
        "1-A": ["1-A POS", "1-K", "1-SA", "1-Z", "Form C", "C-AR", "C-U"],
        "S-1": ["S-1MEF"],
        "F-3": ["F-3ASR"],
        "F-6": ["F-6 POS"]
    }

    # Helper to find form by type
    def find_form(ft):
        for f in forms:
            if f['form_type'] == ft:
                return f
        return None

    # Helper to slugify for filename
    def get_filename(ft):
        # Specific overrides for your project
        overrides = {
            "Form 15": "15.html",
            "Form 4": "4.html",
            "SC 13D": "sc13d.html",
            "10-K": "10k.html",
            "10-Q": "10q.html",
            "8-K": "8k.html",
            "DEF 14A": "def14a.html",
            "424B4": "424b4.html",
            "S-3": "s3.html",
            "S-1": "s1.html",
            "11-K": "11k.html",
            "NT 10-K": "nt10k.html",
            "1-A": "1a.html",
            "F-3": "f3.html",
            "F-6": "f6.html"
        }
        if ft in overrides:
            return overrides[ft]
        s = ft.lower().replace('-', '').replace('/', '').replace(' ', '').replace('.', '')
        return s + ".html"

    final_forms = []
    processed_variants = set()
    for p, vs in merge_map.items():
        processed_variants.update(vs)

    # 1. Update HTML files for parents
    for parent_ft, variant_fts in merge_map.items():
        parent_form = find_form(parent_ft)
        if not parent_form: continue
        
        variant_forms = [find_form(v) for v in variant_fts if find_form(v)]
        if not variant_forms: continue
        
        parent_file = get_filename(parent_ft)
        parent_path = os.path.join(filings_dir, parent_file)
        
        if os.path.exists(parent_path):
            with open(parent_path, 'r', encoding='utf-8') as f:
                soup = BeautifulSoup(f.read(), 'html.parser')
            
            # Remove existing variants section if any
            existing = soup.find('article', id='variants')
            if existing: existing.decompose()
            
            # Create NEW variants section
            v_section = soup.new_tag('article', **{'class': 'content-section', 'id': 'variants'})
            
            header = soup.new_tag('div', **{'class': 'part-header'})
            header.string = "TECHNICAL VARIANTS"
            v_section.append(header)
            
            h2 = soup.new_tag('h2')
            h2.string = "Related Forms & Specialized Variants"
            v_section.append(h2)
            
            grid = soup.new_tag('div', **{'class': 'item-grid', 'style': 'margin-top: 2rem;'})
            
            for v in variant_forms:
                card = soup.new_tag('div', **{'class': 'item-card', 'style': 'border-left: 4px solid var(--accent-blue);'})
                
                v_badge = soup.new_tag('span', **{'class': 'item-num', 'style': 'background: var(--bg-hover); color: var(--accent-blue);'})
                v_badge.string = v['form_type']
                card.append(v_badge)
                
                v_h3 = soup.new_tag('h3')
                v_h3.string = v['full_name']
                card.append(v_h3)
                
                v_p = soup.new_tag('p')
                v_p.string = v['what']
                card.append(v_p)
                
                grid.append(card)
            
            v_section.append(grid)
            
            # Insert before signatures
            sigs = soup.find('article', id='signatures')
            if sigs:
                sigs.insert_before(v_section)
            else:
                body = soup.find('main') or soup.find('body')
                if body: body.append(v_section)
                
            # Update TOC
            toc = soup.select_one('.filing-toc nav')
            if toc and not toc.find('a', href='#variants'):
                new_link = soup.new_tag('a', href='#variants')
                new_link.string = "Related Forms"
                sig_link = toc.find('a', href='#signatures')
                if sig_link: sig_link.insert_before(new_link)
                else: toc.append(new_link)
            
            with open(parent_path, 'w', encoding='utf-8') as f:
                f.write(str(soup))
            print(f"Merged variants into {parent_file}")
            
            # Delete variant files
            for v_ft in variant_fts:
                v_file = get_filename(v_ft)
                v_path = os.path.join(filings_dir, v_file)
                if os.path.exists(v_path) and v_file != parent_file:
                    os.remove(v_path)
                    print(f"  Deleted {v_file}")

    # 2. Rebuild data.json
    for f in forms:
        if f['form_type'] not in processed_variants:
            final_forms.append(f)
            
    full_data['forms'] = final_forms
    full_data['total_forms'] = len(final_forms)
    with open(data_path, 'w', encoding='utf-8') as f:
        json.dump(full_data, f, indent=2)
    print(f"Data.json updated: {len(final_forms)} forms remaining.")

if __name__ == "__main__":
    merge_filings()
